...
public class Lifetime_Sales_Tab extends JFrame {
	...
	public static JTable tempTable2;
	public static JScrollPane scrollPane_3;
	//for the third table that will read the VARIABLE costs
	
	public static DefaultTableModel dtm4 = new DefaultTableModel(); 
	public static JTable tempTable3;
	public static JScrollPane scrollPane_4;
	//for the fourth table that will read the FIXED costs

	public static String[] savefiles = new String[6];
	public Object[] columns = {"Date", "Day #", "Sales", "Costs", "Profit"};
	public Object[] columns2 = {"Picture", "Product Type", "ProductID", "Price","Itl Qty","Qty Sold","Qty Free","End Qty","Total Sale","Restock?","Performance" };
	public Object[] columns3 = { "Product Type", "Supplier", "Pcs", "Unit Cost", "Misc Costs", "Total Cost", "Date Ordered",
	"Date Arrived" };
	public Object[] columns4 = { "Capital/Equipment", "Qty", "Unit Cost", "Misc Fees", "Total Cost", "Date Purchased",
	"Supplier" };
	...
	public Lifetime_Sales_Tab() {
		...
		if(varCosts == null) { // To make sure information is not put into table a second time
			...
			// here)
			fileMethods.readIntoTable(savefiles[2], columns, dtm, varCosts);
		}
		...
		//SEPT 10: https://stackoverflow.com/questions/16801382/make-a-column-non-editable-in-a-jtable#:~:text=Right%2Dclick%20on%20the%20table,to%20make%20it%20non%2Deditable.
		//COlumns 2, 3, and 4 are not editable (sales, costs, and profit columns): index 0 and 1 are editable
		varCosts = new JTable() {
			@Override
...
//		contentPane.add(scrollPane_2);
		...
		//Temp table to read costs
		dtm3.setColumnIdentifiers(columns3);
		scrollPane_3 = new JScrollPane();
		tempTable2 = new JTable();
		scrollPane_3.setViewportView(tempTable2);
		tempTable2.setModel(dtm3);
		
		//Temp table to read costs
		dtm4.setColumnIdentifiers(columns4);
		scrollPane_4 = new JScrollPane();
		tempTable3 = new JTable();
		scrollPane_4.setViewportView(tempTable3);
		tempTable3.setModel(dtm4);		
		
		
		for(int i = 0; i < varCosts.getRowCount(); i++) {
			String date = varCosts.getValueAt(i, 0).toString() + "_";
			String day = varCosts.getValueAt(i, 1).toString() + "_";
			String fileName = date + day;
			
			//These methods already have the back-up process to create the text file if it does not exist
			fileMethods.readIntoTable(fileName + "Sales", columns2, dtm2, tempTable);
			fileMethods.readIntoTable(fileName + "VariableCosts", columns3, dtm3, tempTable2);
			fileMethods.readIntoTable(fileName + "FixedCosts", columns4, dtm4, tempTable3);
			
			float sales = fileMethods.getTotal(tempTable, 8);
			//add variable and fixed costs together
			float costs = fileMethods.getTotal(tempTable2, 5) + fileMethods.getTotal(tempTable3, 4);
			
			System.out.println(sales + " " + costs);
			
			//Used strinv.valueof() to try to add peso symbol BUT i still didnt make the calculation  methods
			//remove the peso symbol
			varCosts.setValueAt(String.valueOf(sales), i, 2);
			varCosts.setValueAt(String.valueOf(costs), i, 3);
			varCosts.setValueAt(String.valueOf(sales - costs), i, 4);
		...
		TableColumn columnName = varCosts.getColumnModel().getColumn(1);
		...
		JLabel labeel = new JLabel("Lifetime Sales");
...
//		label_9.setIcon(new ImageIcon(varCosts.class.getResource("vertical line.png")));
//		label_9.setBounds(138, -34, 19, 710);
//		contentPane.add(label_9);
//		
//		JLabel label_10 = new JLabel("");
//		label_10.setIcon(new ImageIcon(varCosts.class.getResource("horizontal line.png")));
...
//		contentPane.add(label_2);
		...
		btnViewCurrentSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(null,
						"The data in the table above is currently being saved to '" + savefiles[2] + ".txt'");
			}
		});
		...
		lblSumVariableCost_1.setBounds(869, 459, 194, 45);
		contentPane.add(lblSumVariableCost_1);
		lblSumVariableCost_1.setText("\u20B1" + String.valueOf(fileMethods.getTotal(varCosts, 4)));
		...
		btnLifetime.setForeground(Color.BLUE);
		btnLifetime.setFont(new Font("Tahoma", Font.BOLD | Font.ITALIC, 12));
...
//                }
				...
				if ( (tme.getColumn() == 0 || tme.getColumn() == 1) ) {
					String date = varCosts.getValueAt(tme.getFirstRow(), 0).toString() + "_";
					String day = varCosts.getValueAt(tme.getFirstRow(), 1).toString() + "_";
					String fileName = date + day;
					
					//These methods already have the back-up process to create the text file if it does not exist
					//SHOULD I use regex to make a pattern finding if "date" has "20" + the other numbers
					//and "day" has "D" + (integer)
					fileMethods.readIntoTable(fileName + "Sales", columns2, dtm2, tempTable);
					fileMethods.readIntoTable(fileName + "VariableCosts", columns3, dtm3, tempTable2);
	...
	public void save() {
		if(search.getText().length() < 0) {
			...
			if (save == JOptionPane.YES_OPTION) {
				...
				try {
					File f = new File(savefiles[2] + ".txt"); // creates text file

...
public class Inventory_Tab extends JFrame {
	...
	public Object[] columns = { "Picture", "Product Type", "ProductID", "Price", "Itl Qty", "Qty Sold", "Qty Free",
			"End Qty", "Total Sale", "Restock?", "Performance" };
	...
	public Inventory_Tab() {
		...
		if (varCosts == null) { // To make sure information is not put into table a second time
								...
								// here)
			fileMethods.readIntoTable(savefiles[1], columns, dtm, varCosts);
		}
		...
		varCosts = new JTable() {
			...
			public boolean isCellEditable(int row, int column) {
				// 10th and 11th columns (restock & performance) are NOT editable
				return column != 9 && column != 10 ? true : false;
			}
		};
...
//				row[7] = "\u20B1" + "0";
		...
		JLabel labeel = new JLabel("Inventory DURING Convention");
...
//		contentPane.add(label_2);
		...
		btnViewCurrentSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(null,
						"The data in the table above is currently being saved to '" + savefiles[1] + ".txt'");
			}
		});
		...
		lblSumVariableCost.setBounds(505, 462, 194, 45);
		...
		lblSumVariableCost_1.setBounds(689, 462, 194, 45);
		...
		btnDuringCon.setForeground(Color.BLUE);
		btnDuringCon.setFont(new Font("Tahoma", Font.BOLD | Font.ITALIC, 12));
...
//                }
				...
				if (tme.getColumn() == 3) {
					// price * total sale
					float totSale = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 3).toString())
							* Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 7).toString());
					varCosts.setValueAt(String.valueOf(totSale), tme.getFirstRow(), 8);

					lblSumVariableCost_1.setText("\u20B1" + String.valueOf(getTotal()));
				}

				if (tme.getColumn() == 4 || tme.getColumn() == 5 || tme.getColumn() == 6) {
					int initialQ = Integer.parseInt(varCosts.getValueAt(tme.getFirstRow(), 4).toString());
					int soldQ = Integer.parseInt(varCosts.getValueAt(tme.getFirstRow(), 5).toString());
					int freeQ = Integer.parseInt(varCosts.getValueAt(tme.getFirstRow(), 6).toString());

					int endQ = initialQ - (soldQ + freeQ);
					float totSale = endQ * Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 3).toString());

					varCosts.setValueAt(String.valueOf(endQ), tme.getFirstRow(), 7);
					varCosts.setValueAt(String.valueOf(totSale), tme.getFirstRow(), 8);

					lblSumVariableCost_1.setText("\u20B1" + String.valueOf(getTotal()));

					// intBitstoFloat: need endQ and initialQ to become floats to do proper division
					// (integer / integer returns integer)
					float performance = (Float.intBitsToFloat(endQ) / Float.intBitsToFloat(initialQ)) * 100;
					float restock = (1 - (performance / 100)) * 100;
					varCosts.setValueAt(String.valueOf(restock) + "%", tme.getFirstRow(), 9);
					varCosts.setValueAt(String.valueOf(performance) + "%", tme.getFirstRow(), 10);
					// update performance
...
//				total += 0;
//			} else {
			total += Float.parseFloat(varCosts.getValueAt(i, 8).toString());
//			}
	...
	public void save() {
		if (search.getText().length() < 0) {
			...
			if (save == JOptionPane.YES_OPTION) {
				...
				try {
					File f = new File(savefiles[1] + ".txt"); // creates text file

...
public class VariableCosts_Tab extends JFrame {

	private JPanel contentPane;
	public static EditableTableModel dtm = new EditableTableModel();
	public static JTable varCosts;
	public static JScrollPane scrollPane_1;

	public static String[] savefiles = new String[6];
	public Object[] columns = { "Product Type", "Supplier", "Pcs", "Unit Cost", "Misc Costs", "Total Cost",
			"Date Ordered", "Date Arrived" };
	...
	public VariableCosts_Tab() {
		...
		if(varCosts == null) { // To make sure information is not put into table a second time
			...
			// here)
			fileMethods.readIntoTable(savefiles[0], columns, dtm, varCosts);
		}
		...
		// Add choices to each comboBox column
		fileMethods.setUpComboBoxCells(varCosts, varCosts.getColumnModel().getColumn(0), "VarCostsSizeandTypes");
		fileMethods.setUpComboBoxCells(varCosts, varCosts.getColumnModel().getColumn(1), "Suppliers");
		JComboBox prices = new JComboBox();
		...
		JLabel labeel = new JLabel("Variable Costs + Order History");
...
//		contentPane.add(label_2);
		...
		btnViewCurrentSave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(null,
						"The data in the table above is currently being saved to '" + savefiles[0] + ".txt'");
			}
		});
		...
		JLabel lblSumVariableCost = new JLabel("Sum Variable Cost:");
		...
		btnVariable.setForeground(Color.BLUE);
		btnVariable.setFont(new Font("Tahoma", Font.BOLD | Font.ITALIC, 12));
...
//                }
				...
				if (!change) {
					...
					// Can only edit product type, pcs, misc cost, or total cost) (column #0,2,4,5)
					if ((tme.getColumn() == 0 || tme.getColumn() == 2 || tme.getColumn() == 4 || tme.getColumn() == 5)
							&& varCosts.getValueAt(tme.getFirstRow(), 0).toString().contains("Sticker")
							&& change == false) {
						float newTotal = 0;
						float newUnitCost = 0;

						float pcs = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 2).toString());
						float miscCost = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 4).toString());
						float totalCost = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 5).toString());

						// raw cost + miscellaneous cost
						newUnitCost = (totalCost - miscCost) / pcs;
//					}

						change = true;
						varCosts.setValueAt(String.valueOf(newUnitCost), tme.getFirstRow(), 3);
						lblSumVariableCost_1.setText("\u20B1" + String.valueOf(getTotal()));
					}

					// If you edit the pieces, unit cost, or misc cost
					// OR if you change the product from sticker to regular product
					else if (tme.getColumn() == 0 || tme.getColumn() == 2 || tme.getColumn() == 3
							|| tme.getColumn() == 4 && change == false) {

						float newTotal = 0;

						float pcs = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 2).toString());
						float unitCost = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 3).toString());
						float miscCost = Float.parseFloat(varCosts.getValueAt(tme.getFirstRow(), 4).toString());
...
//						}
						...
						change = true;
						varCosts.setValueAt(String.valueOf(newTotal), tme.getFirstRow(), 5);

						// Update text total with peso symbol outside
						lblSumVariableCost_1.setText("\u20B1" + String.valueOf(getTotal()));
					}

					else if (tme.getColumn() == 5) {
						// Update text total with peso symbol outside
						lblSumVariableCost_1.setText("\u20B1" + String.valueOf(getTotal()));
					}
				}
...
//				total += 0;
//			} else {
			total += Float.parseFloat(varCosts.getValueAt(i, 5).toString());
//			}
	...
	public void save() {
		if (search.getText().length() < 0) {
			...
			if (save == JOptionPane.YES_OPTION) {
				...
				try {
					File f = new File(savefiles[0] + ".txt"); // creates text file
